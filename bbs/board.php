...
// g5pro: 상단·하단 내용을 미리 잡아 두었다가 뷰가 제자리에 넣는다.
// 하단도 지금 잡는다 — 아래 목록/읽기가 화면 전체를 이미 출력해 버리기 때문.
g5_pro_capture_start(); include_once(G5_BBS_PATH.'/board_head.php'); g5_pro_capture_end('content_head');
g5_pro_capture_start(); include_once(G5_BBS_PATH.'/board_tail.php'); g5_pro_capture_end('content_tail');

// 전체목록보이기 + 읽기가 겹치면, 순정은 두 스킨을 잇달아 출력한다.
// blade 는 화면이 하나이므로 목록을 먼저 "수집"해 두었다가 읽기 화면 아래에 붙인다. (g5pro)
$g5_pro_show_list = ($member['mb_level'] >= $board['bo_list_level'] && $board['bo_use_list_view']) || empty($wr_id);
if (isset($wr_id) && $wr_id && $g5_pro_show_list) {
    $GLOBALS['g5_pro_collect_list'] = true;
    include_once (G5_BBS_PATH.'/list.php');
    $GLOBALS['g5_pro_collect_list'] = false;
}
...
if ($g5_pro_show_list && empty($wr_id))
    include_once (G5_BBS_PATH.'/list.php');
