...
for ($i = 1; $i <= ITEM_CNT; $i++) {
    ...
    for ($k = 0; $k < ORDER_PER_IT; $k++) {
        ...
        $odRows[] = "($od_id, '$mb_id', '$name', '$mb_id@test.local', '', '$hp',"
                  . " '060', '00', '서울시 강남구 테스트로 $m', '{$m}층', "
                  . " '$name', '$hp', '서울시 강남구 테스트로 $m', '{$m}층',"
                  . " '', 1, $amount, $send, $receipt, $receipt_time, $misu,"
                  . " '', '', '$status', '무통장', '0', 0, '', '', $time, '', '127.0.0.1')";
        $ctRows[] = "($od_id, '$mb_id', '$it_id', '$it_name', '$status', '',"
                  . " $price, $stock_use, '', $qty, '', $time, '127.0.0.1', 1, $time)";
    }
    ...
    $db->query("INSERT INTO g5_shop_order
        ...
        VALUES ".implode(',', $odRows)) or exit("주문 INSERT 실패($it_id): ".$db->error."\n");
    ...
    $db->query("INSERT INTO g5_shop_cart
        ...
        VALUES ".implode(',', $ctRows)) or exit("카트 INSERT 실패($it_id): ".$db->error."\n");

    if ($i % 10 === 0) {
        $db->commit();
        echo "  주문 ".number_format($seq)."건...\n";
    }
}
$db->commit();
$db->autocommit(true);

printf("완료: 회원 %d · 상품 %d · 주문 %s건 (%.1f초)\n",
    MEMBER_CNT, ITEM_CNT, number_format($seq), microtime(true) - $t0);
