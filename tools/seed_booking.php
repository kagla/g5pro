...
// 객실 20개 — 이름, 소개, 기준/최대 인원, 인원추가요금, 실수, 주중가, 주말 가산율(%)
$SEED_ROOMS = array(
    array('디럭스 스파',      '넓은 창으로 계곡이 내려다보이는 스파 객실입니다. 2인용 월풀 욕조와 캡슐 커피머신을 갖췄습니다.', 2, 4, 20000, 3, 150000, 45),
    array('패밀리 온돌',      '온 가족이 함께 눕는 좌식 온돌방입니다. 이불 4채와 넉넉한 주방이 준비되어 있습니다.',              4, 8, 15000, 2, 180000, 40),
    array('오션뷰 복층',      '거실 통창과 다락 침실이 있는 복층 객실입니다. 2층 창가에서 바다가 한눈에 들어옵니다.',            2, 6, 20000, 2, 220000, 50),
    array('독채 풀빌라',      '마당과 개인 수영장을 통째로 쓰는 독채입니다. 바비큐 데크와 야외 샤워장이 딸려 있습니다.',        4, 8, 30000, 1, 350000, 55),
    array('커플 자쿠지',      '커플 전용 객실로 창가 자쿠지와 무드 조명, 블루투스 스피커를 두었습니다.',                        2, 2,     0, 4, 130000, 50),
    array('마운틴뷰 테라스',  '전용 테라스에서 앞산 능선을 마주 보는 객실입니다. 아침 안개가 특히 좋습니다.',                    2, 4, 15000, 3, 120000, 40),
    array('프리미엄 스위트',  '침실과 거실이 분리된 가장 넓은 객실입니다. 킹 침대와 6인용 다이닝 테이블을 갖췄습니다.',          2, 6, 25000, 1, 280000, 50),
    array('가든 독채',        '잔디 마당을 낀 단층 독채입니다. 문을 열면 바로 정원이라 아이들이 뛰어놀기 좋습니다.',            4, 6, 20000, 2, 200000, 45),
    array('다락방 로맨틱',    '천창으로 별이 보이는 다락 객실입니다. 낮은 층고가 주는 아늑함이 매력입니다.',                    2, 3, 15000, 2,  95000, 40),
    array('리버뷰 원룸',      '강가 산책로 바로 앞 원룸형 객실입니다. 짧게 머무는 여행에 알맞습니다.',                          2, 3, 12000, 4,  85000, 35),
    array('우드 캐빈',        '통나무로 지은 산장형 객실입니다. 벽난로와 흔들의자가 겨울에 특히 인기입니다.',                    2, 4, 18000, 3, 140000, 45),
    array('그랜드 패밀리',    '방 2개에 욕실 2개를 둔 대가족용 객실입니다. 세탁기와 건조기도 객실 안에 있습니다.',              6, 8, 20000, 1, 300000, 40),
    array('선셋 오션 스위트', '해가 지는 방향으로만 창을 낸 객실입니다. 저녁 시간대의 노을이 그대로 들어옵니다.',                2, 4, 25000, 2, 260000, 50),
    array('힐링 한옥채',      '대청마루가 있는 한옥 객실입니다. 툇마루에 앉아 마당을 바라보며 쉴 수 있습니다.',                  2, 5, 20000, 2, 190000, 45),
    array('스카이 루프탑',    '옥상 전용 데크를 쓰는 최상층 객실입니다. 밤에는 데크 조명 아래 야경이 펼쳐집니다.',              2, 4, 20000, 2, 210000, 50),
    array('포레스트 트윈',    '숲을 향해 트윈 침대를 나란히 둔 객실입니다. 친구·동료 여행에 알맞습니다.',                        2, 4, 15000, 4, 110000, 35),
    array('프라이빗 풀 스파', '실내 온수풀과 스파를 함께 갖춘 객실입니다. 계절에 상관없이 물놀이가 가능합니다.',                2, 6, 30000, 1, 330000, 55),
    array('노을 정원채',      '작은 정원을 낀 독채로, 화로대와 야외 테이블이 준비되어 있습니다.',                                4, 6, 20000, 2, 175000, 45),
    array('별빛 캠핑동',      '글램핑 감성의 텐트형 객실입니다. 침대와 에어컨을 갖춰 사계절 이용할 수 있습니다.',                2, 4, 12000, 4,  90000, 40),
    array('라운지 복층 스위트','1층 라운지와 2층 침실로 나뉜 복층 스위트입니다. 소규모 모임 장소로도 좋습니다.',                 4, 8, 25000, 1, 240000, 50),
);

// 부가상품 12개 — 이름, 가격, 최대수량 (기존 ba_id=36 '조식' 과 이름이 겹치지 않게 한다)
$SEED_ADDONS = array(
    array('바베큐 세트',        30000, 5),
    array('숯+그릴 대여',       15000, 4),
    array('조식 세트(2인)',     18000, 8),
    array('침구 추가',          10000, 6),
    array('픽업 서비스(편도)',  20000, 2),
    array('불멍 세트',          25000, 3),
    array('와인 웰컴 세트',     35000, 4),
    array('케이크 서프라이즈',  28000, 2),
    array('반려동물 동반',      20000, 2),
    array('수영장 이용권',      12000, 10),
    array('조식 도시락',         9000, 8),
    array('낚시대 대여',         5000, 6),
);
...
foreach ($SEED_ROOMS as $i => $r) {
    list($subject, $desc, $base, $max, $person_price, $count, $weekday, $weekend_rate) = $r;
    $weekend = (int)(round($weekday * (100 + $weekend_rate) / 100 / 1000) * 1000);
...
// 앞으로 3개월 안에서:
//   - 다가오는 달 11~24일 = 성수기. 객실 절반(홀수 번째)의 요금을 주중가 ×1.5~2 로 덮는다
...
foreach ($room_ids as $n => $br_id) {
    ...
    foreach ($peak_days as $date) {
        ...
        $cal[$br_id][$date] = array('price' => (int)(round($base * $rate / 1000) * 1000), 'count' => -1);
    }
}
