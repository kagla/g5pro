...
{
    return array('상품코드', '상품명', '분류코드', '노출', 'SKU코드', '옵션', '판매가', '재고', '바코드', 'SKU사용');
}
...
function cart_csv_export_rows()
{
    ...
    while ($r = sql_fetch_array($result)) {
        ...
        $rows[] = array(cart_csv_guard($r['it_code']), cart_csv_guard($r['it_name']),
            cart_csv_guard((string)$r['ca_codes']), $r['it_show'],
...
function cart_csv_parse($path, &$errors)
{
    ...
    while (($line = fgetcsv($fp)) !== false) {
        ...
        foreach (array('상품코드', '상품명', '분류코드', 'SKU코드', '옵션', '바코드') as $guarded_key) {
...
{
    $cell = trim($cell);
    if ($cell === '') return array();
    $ids = array();
    foreach (explode(',', $cell) as $code) {
        $code = trim($code);
        if ($code === '') continue;
        $row = cart_category_get_by_code($code);
        if (!$row) return null;
        $ids[] = (int)$row['ca_id'];
    }
    return $ids ? $ids : null;
...
function cart_csv_summary($rows)
{
    ...
    foreach ($rows as $row) {
        ...
        if (cart_csv_category_ids($row['분류코드']) === null) {
            $sum['errors'][] = $row['_line'].'행: 없는 분류코드가 있습니다: '.$row['분류코드'];
            continue;
        }
        $code = $row['상품코드'];
...
// 500행 청크 반영. summary 와 완전히 같은 순서·같은 조건(옵션 형식 → 분류코드 존재 → 신규상품
// 상품명 필수 → SKU 소유권 충돌)으로 건너뛸 행을 먼저 가려낸 뒤에만 실제로 쓴다 — 미리보기
...
function cart_csv_apply($rows, $who)
{
    ...
    foreach (array_chunk($rows, 500) as $chunk) {
        ...
        foreach ($chunk as $row) {
            ...
            $csv_ca_ids = cart_csv_category_ids($row['분류코드']);
            if ($csv_ca_ids === null) continue;
