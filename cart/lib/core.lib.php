...
// 목록 한 페이지에 몇 줄 — 표로 된 목록은 손님 화면이든 관리자든 이 값 하나를 쓴다.
//
// 화면마다 따로 적어 두었더니 어떤 표는 30줄, 어떤 표는 20줄이 되어 페이지를 넘길 때마다
// 눈이 다시 적응해야 했다. 숫자에 뜻이 있는 것이 아니라 "같아야 한다" 는 것이 뜻이라
// 값 하나를 한 곳에 둔다.
//
// 상품 목록·찜 목록(cart/list.php·wish.php)은 표가 아니라 격자라 이 값을 안 쓴다 —
// 거기 24 는 열 수(2·3·4)의 공배수여서 마지막 줄이 안 비는 값이다. 20 으로 맞추면
// 3열에서 마지막 줄에 두 칸이 빈다.
function cart_rows_per()
{
    return 20;
}
